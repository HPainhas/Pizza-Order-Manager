...
public final class PizzaOrderManager {
    ...
    private static void getPriceMap(String fileName,
        ...
        while (!input.atEOS()) {
            ...
            int record = fileLine.indexOf(',');
            String mapValue = fileLine.substring(0, record);
            String mapValue2 = fileLine.substring(record + 1);
            int mapValue3 = Integer.parseInt(mapValue2);
            priceMap.add(mapValue, mapValue3);
        }
    ...
    private static int getOneOrder(SimpleReader input,
        ...
        int sum = sizePriceMap.value(input.nextLine());
        String order = input.nextLine();
        while (!order.equals("")) {
            sum = sum + toppingPriceMap.value(order);
